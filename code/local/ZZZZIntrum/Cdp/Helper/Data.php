...
class ZZZZIntrum_Cdp_Helper_Data extends Mage_Core_Helper_Abstract {

    function saveLog(Mage_Sales_Model_Quote $quote, ZZZZIntrum_Cdp_Helper_Api_Classes_IntrumRequest $request, $xml_request, $xml_response, $status, $type) {
        ...
        $intrum_model = Mage::getModel('intrum/intrum');
        $intrum_model->setData($data);
        $intrum_model->save();
    }
    ...
    public function saveStatusToOrder($orderId, $intrum_status, ZZZZIntrum_Cdp_Helper_Api_Classes_IntrumResponse $IntrumResponse) {
    ...
    function CreateMagentoShopRequest(Mage_Sales_Model_Quote $quote) {
        ...
        $request = new ZZZZIntrum_Cdp_Helper_Api_Classes_IntrumRequest();
        ...
        if (Mage::getStoreConfig('intrum/api/plugincheckouttype', Mage::app()->getStore()) == 'amasty' && !empty($_POST["billing"]["dob"])) {
        ...
        if (Mage::getStoreConfig('intrum/api/plugincheckouttype', Mage::app()->getStore()) == 'amasty' && !empty($_POST["method"]) && $_POST["method"] == 'guest') {
            $request->setCustomerReference(uniqid("guest_"));
        }

        $request->setFirstName((String)$quote->getBillingAddress()->getFirstname());
        if (Mage::getStoreConfig('intrum/api/plugincheckouttype', Mage::app()->getStore()) == 'amasty' && !empty($_POST["billing"]["firstname"])) {
            $request->setFirstName((String)$_POST["billing"]["firstname"]);
        }

        $request->setLastName((String)$quote->getBillingAddress()->getLastname());
        if (Mage::getStoreConfig('intrum/api/plugincheckouttype', Mage::app()->getStore()) == 'amasty' && !empty($_POST["billing"]["lastname"])) {
            $request->setLastName((String)$_POST["billing"]["lastname"]);
        }

        $request->setFirstLine(trim((String)$quote->getBillingAddress()->getStreetFull()));
        if (Mage::getStoreConfig('intrum/api/plugincheckouttype', Mage::app()->getStore()) == 'amasty' && !empty($_POST["billing"]["street"])) {
            $street = '';
        ...
        if (Mage::getStoreConfig('intrum/api/plugincheckouttype', Mage::app()->getStore()) == 'amasty' && !empty($_POST["billing"]["country_id"])) {
            $request->setCountryCode((String)$_POST["billing"]["country_id"]);
        }

        $request->setPostCode((String)$quote->getBillingAddress()->getPostcode());
        if (Mage::getStoreConfig('intrum/api/plugincheckouttype', Mage::app()->getStore()) == 'amasty' && !empty($_POST["billing"]["postcode"])) {
            $request->setPostCode($_POST["billing"]["postcode"]);
        }

        $request->setTown((String)$quote->getBillingAddress()->getCity());
        if (Mage::getStoreConfig('intrum/api/plugincheckouttype', Mage::app()->getStore()) == 'amasty' && !empty($_POST["billing"]["city"])) {
            $request->setTown($_POST["billing"]["city"]);
        }

        $request->setFax((String)trim($quote->getBillingAddress()->getFax(), '-'));
        if (Mage::getStoreConfig('intrum/api/plugincheckouttype', Mage::app()->getStore()) == 'amasty' && !empty($_POST["billing"]["fax"])) {
        ...
        if (Mage::getStoreConfig('intrum/api/plugincheckouttype', Mage::app()->getStore()) == 'amasty' && !empty($_POST["billing"]["company"])) {
        ...
        if (Mage::getStoreConfig('intrum/api/plugincheckouttype', Mage::app()->getStore()) == 'amasty' && !empty($_POST["billing"]["telephone"])) {
            $request->setTelephonePrivate(trim($_POST["billing"]["telephone"], '-'));
        }

        $request->setEmail((String)$quote->getBillingAddress()->getEmail());
        if (Mage::getStoreConfig('intrum/api/plugincheckouttype', Mage::app()->getStore()) == 'amasty' && !empty($_POST["billing"]["email"])) {
        ...
        if (Mage::getStoreConfig('intrum/api/plugincheckouttype', Mage::app()->getStore()) == 'amasty' && !empty($_POST["billing"]["prefix"])) {
        ...
        if (!$quote->isVirtual()) {
            ...
            if (Mage::getStoreConfig('intrum/api/plugincheckouttype', Mage::app()->getStore()) == 'amasty' && empty($_POST["shipping"]["same_as_billing"])) {
    ...
    function CreateMagentoShopRequestPaid(Mage_Sales_Model_Order $order, $paymentmethod, $amount) {
        ...
        if (Mage::getStoreConfig('intrum/api/plugincheckouttype', Mage::app()->getStore()) == 'amasty' && empty($b)) {
        ...
        if (Mage::getStoreConfig('intrum/api/plugincheckouttype', Mage::app()->getStore()) == 'amasty' && empty($g)) {
        ...
        $extraInfo["Name"] = 'ORDERAMOUNT';
        $extraInfo["Value"] = number_format($amount, 2, '.', '');
        $request->setExtraInfo($extraInfo);
        ...
        if ($order->canShip()) {
            ...
            if ($order->getShippingAddress()->getCompany() != '' && Mage::getStoreConfig('intrum/api/businesstobusiness', Mage::app()->getStore()) == 'enable') {
                $extraInfo["Name"] = 'DELIVERY_COMPANYNAME';
